...
--#region Webhook Sender (วิธีทำ Webhook)
	...
	pcall(function()
		...
		local data = {
			...
			["embeds"] = {
				{
					["author"] = {
						["name"] = "[✨UPD 8] Anime Adventure Status",
					...
					["fields"] = {
						{
							["name"] = "Total Waves:", tostring(waves[2]) .. " <:wave:997136622363627530>",
							["inline"] = true
						}, {
							["name"] = "Recieved Gems:", gems .. " <:gem:997123585476927558>",
							["inline"] = true
						}, {
                            ["name"] = "Recieved XP:", XP .. " 🧪",
                            ["inline"] = true
                        }, {
                            ["name"] = "Total Time:", tostring(ttime[2]) .. " ⏳",
                            ["inline"] = true
                        }, {
                            ["name"] = "Current Gems:", tostring(game.Players.LocalPlayer._stats.gem_amount.Value).." <:gem:997123585476927558>",
                            ["inline"] = true
                        }, {
                            ["name"] = "Current Level:", tostring(game.Players.LocalPlayer.PlayerGui.spawn_units.Lives.Main.Desc.Level.Text),
                            ["inline"] = true
                        }, {
                            ["name"] = "คนทำ", "! liz#4863",
                            ["inline"] = true
...
--#endregion
    ...
    local win = DiscordLib:Window("Lizard Shop [Anime Adventures]"..versionx.." - "..exec)
    ...
    if game.PlaceId == 8304191830 then
        ...
        local slectworld = autofrmserver:Channel(" - Select Item")
...
--#region Select Units Tab
        ...
        unitselecttab:Button("Select Units", function()
...
--#region Select world tab
        ...
        getgenv().diff = slectworld:Dropdown("Select Mode", {"Normal", "Hard"}, getgenv().difficulty, function(diff)
...
--#region Auto Farm Tab
        ...
        autofarmtab:Toggle("Auto Infinity Castle", getgenv().AutoFarmIC, function(bool)
        ...
        ---- 
        autofarmtab:Textbox("Select Wave for Auto Sell", tostring(getgenv().sellatwave), false, function(t)
            getgenv().sellatwave = tonumber(t)
            updatejson()
        end)

		local webhooktab = webhookserver:Channel("Webhook")
		webhooktab:Label("Webhook sends notification in discord everytime when Game is Finished!")
		
		local webhoolPlaceholder
		if getgenv().weburl == "" then
			webhoolPlaceholder = "Press url here!"
		else
			webhoolPlaceholder = getgenv().weburl
		end
		webhooktab:Textbox("Webhook URL" , webhoolPlaceholder, false, function(web_url)
...
--#region Auto Challenge
        ...
        local worlddrop = autoclngtab:Dropdown("Select Reward", {"star_fruit random","star remnant","gems", "gold"}, getgenv().selectedreward, function(reward)
...
if getgenv().AutoLoadTP == true then
    ...
    if exec == "Synapse X" then
        syn.queue_on_teleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/Chaxrr/Chaxrr-AnimeAdventures-Test/main/.lua'))()")
    else
        queue_on_teleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/Chaxrr/Chaxrr-AnimeAdventures-Test/main/.lua'))()")
    end
